...
/// 
/// This is the type that all `Foo_Ref` generated by using
/// `#[sabi(kind(Prefix)))]` on `Foo` while deriving `StableAbi` wrap.
/// 
/// # Example
/// 
/// ```rust
...
impl<P> PrefixRef<P>{
    ...
    /// 
    /// let prefix: &Module_Prefix = unsafe{ PREFIX_REF.prefix() };
    /// 
