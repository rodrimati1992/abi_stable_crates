...
fn check_null(){
    ...
    let list=list.into_iter().map(Tag::to_checkable).collect::<Vec<CheckableTag>>();

    for elems in &list {
        assert_eq!(
            null_checkable.check_compatible(elems),
            Ok(())
        );

        assert_ne!(
            elems.check_compatible(&null_checkable),
            Ok(())
        );
    }
}