use super::{
    lib_header::AbiHeader,
    root_mod_trait::RootModule,
};
...
impl Display for LibraryError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            ...
            LibraryError::RootModule{err,module_name,version} => {
                writeln!(f, "An error ocurred while loading this library:\t\n{}", module_name)?;
                writeln!(f, "version:\n\t{}", version)?;
...
impl<M: RootModule> IntoRootModuleResult for RResult<M, RBoxError> {
    ...
    fn into_root_module_result(self) -> Result<M, RootModuleError> {
        self.into_result()
            .map_err(RootModuleError::Returned)
    }
}
...
mod tests{
    ...
    fn into_root_module_result_test() {
        ...
        fn test_case(
            ...
            assert_eq!(ok.unwrap().0.to_raw_ptr() as usize, PREFIX.0.to_raw_ptr() as usize);            
