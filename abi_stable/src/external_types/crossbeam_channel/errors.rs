...
impl<T> Display for RTrySendError<T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let msg=match self {
            RTrySendError::Full{..} => 
                "Attempting to send on a full channel",
            RTrySendError::Disconnected{..} => 
...
impl Display for RTryRecvError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let msg=match self {
            RTryRecvError::Empty{..} => 
                "Attempting to recv on an empty channel",
...
impl<T> Display for RSendTimeoutError<T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let msg=match *self {
            RSendTimeoutError::Timeout{..} => 
                "Timed out while attempting to send on a channel",
            RSendTimeoutError::Disconnected{..} => 
                "Attempting to send on a disconnected channel",
        };
        f.pad(msg)
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let msg=match self {
            RRecvTimeoutError::Timeout{..} => 
                "Timed out while attempting to recv on a channel",
