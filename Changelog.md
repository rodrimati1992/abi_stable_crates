...
Turned test functions generated by the `#[sabi(assert_nonexhaustive(..))]` helper attribute into a static assertions.
...
Added these functions, which are const when the `"rust_1_64"` feature is enabled:
- `RCow::as_str`
- `RCow::as_slice`
...
Removed all `rust_*` features up to 1.57.0, making items that require 1.57.0 unconditional
