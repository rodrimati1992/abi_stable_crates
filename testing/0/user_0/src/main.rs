...
use testing_interface_0::{PrefixTypeMod1_Ref, TestingMod_Ref};
...
fn main() -> io::Result<()> {
    ...
    let mods =
        TestingMod_Ref::load_from_directory(&library_path).unwrap_or_else(|e| panic!("{}", e));
...
pub fn run_dynamic_library_tests(mods: TestingMod_Ref) {
    ...
    let val = mods.for_tests()();
    {
        ...
        assert_ne!((&*arc_std) as *const _ as usize, val.arc_address);
    }
    {
        let box_std = val.box_.piped(RBox::into_box);
        assert_ne!((&*box_std) as *const _ as usize, val.box_address);
    }
    {
        let vec_std = val.vec_.piped(RVec::into_vec);
        assert_ne!(vec_std.as_ptr() as usize, val.vec_address);
    }
    {
        let string_std = val.string.piped(RString::into_string);
        assert_ne!(string_std.as_ptr() as usize, val.string_address);
    }
